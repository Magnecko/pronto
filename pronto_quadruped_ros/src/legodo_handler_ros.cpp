...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("leg_odometer");

namespace pronto {
namespace quadruped {

LegodoHandlerBase::LegodoHandlerBase(const rclcpp::Node::SharedPtr& node,
    ...
    leg_odometer_(legodo)
{
    ...
    if (!node->has_parameter(prefix + "downsample_factor"))
    {
      node->declare_parameter(prefix + "downsample_factor", 1);
    }
    downsample_factor_ = node->get_parameter(prefix + "downsample_factor").as_int();

    // Get utime_offset
    if (!node->has_parameter(prefix + "utime_offset"))
    {
      node->declare_parameter(prefix + "utime_offset", 0);
    }
    utime_offset_ = node->get_parameter(prefix + "utime_offset").as_int();

    RCLCPP_INFO_STREAM(node->get_logger(), "[LegodoHandler] downsample_factor = " << downsample_factor_ << "\n" <<
                    "                utime_offset =      " << utime_offset_);

    double r_vx;
    double r_vy;
    double r_vz;
    if(!node->get_parameter(prefix + "r_vx", r_vx)){
        RCLCPP_WARN_STREAM(node->get_logger(), "Could not retrieve r_vx from parameter server."
                        <<  " Setting to default.");
        r_vx = 0.1;
    }
    if(!node->get_parameter(prefix + "r_vy", r_vy)){
        RCLCPP_WARN_STREAM(node->get_logger(), "Could not retrieve r_vy from parameter server."
                        <<  " Setting to default.");
        r_vy = 0.1;
    }
    if(!node->get_parameter(prefix + "r_vz", r_vz)){
        RCLCPP_WARN_STREAM(node->get_logger(), "Could not retrieve r_vz from parameter server."
                        <<  " Setting to default.");
        r_vz = 0.1;
    }
    ...
    // Get base_frame
    if (!node->has_parameter("base_link_name"))
    {
      node->declare_parameter("base_link_name", "base");
    }
    base_link_name_ = node->get_parameter("base_link_name").as_string();
    foot_names_.resize(4);
    foot_names_[0] = node->declare_parameter<std::string>("LF_FOOT_name", "LF_FOOT");
    foot_names_[1] = node->declare_parameter<std::string>("RF_FOOT_name", "RF_FOOT");
    foot_names_[2] = node->declare_parameter<std::string>("LH_FOOT_name", "LH_FOOT");
    foot_names_[3] = node->declare_parameter<std::string>("RH_FOOT_name", "RH_FOOT");
    ...
    if(debug_){
        for(int i=0; i<4; i++){
            ...
            grf_debug_.push_back(node->create_publisher<geometry_msgs::msg::WrenchStamped>(leg_names[i]+ "_grf", 10));
            grf_in_foot_frame_debug_.push_back(node->create_publisher<geometry_msgs::msg::WrenchStamped>(leg_names[i]+ "_grf_in_foot", 10));
        }

        vel_raw_ = node->create_publisher<geometry_msgs::msg::TwistStamped>("vel_raw", 10);
        stance_pub_ = node->create_publisher<pronto_msgs::msg::QuadrupedStance>("stance", 10);
        prior_accel_debug_ = node->create_publisher<geometry_msgs::msg::AccelStamped>("prior_accel", 10);
        prior_joint_accel_debug_ = node->create_publisher<sensor_msgs::msg::JointState>("prior_joint_accel", 10);
        prior_velocity_debug_ = node->create_publisher<geometry_msgs::msg::TwistStamped>("prior_vel", 10);
        vel_sigma_bounds_pub_ = node->create_publisher<pronto_msgs::msg::VelocityWithSigmaBounds>("vel_sigma_bounds", 10);
    ...
    output_log_to_file_ = node->declare_parameter<bool>(prefix + "output_log_to_file", true);
    ...
    RCLCPP_INFO_STREAM(node->get_logger(), "[LegodoHandler] Publishing debug topics:       " << std::boolalpha << debug_ << "\n" <<
...
LegodoHandlerBase::Update* LegodoHandlerBase::computeVelocity(){
  if(debug_){
      ...
      wrench_msg_.header.stamp = rclcpp::Time(nsec_);
      ...
      prior_accel_msg.header.stamp = rclcpp::Time(nsec_);
      ...
      prior_vel_msg.header.stamp = rclcpp::Time(nsec_);
                                    ...
                                    cov_legodo_))
  {
      if(debug_){
          ...
          vel_sigma_bound_msg_.header.stamp = rclcpp::Time(nsec_);
          tf2::toMsg(r_legodo_, vel_sigma_bound_msg_.plus_one_sigma);
          tf2::toMsg(xd_ + r_legodo_, vel_sigma_bound_msg_.velocity_plus_one_sigma);
          tf2::toMsg(xd_ - r_legodo_, vel_sigma_bound_msg_.velocity_minus_one_sigma);
          ...
          twist.header.stamp = rclcpp::Time(nsec_);
    ...
    if (debug_) RCLCPP_WARN(LOGGER, "[LegodoHandlerBase::computeVelocity] Could not estimate velocity");
  }
  return nullptr;
}
...
LegodoHandlerROS::Update* LegodoHandlerROS::processMessage(const sensor_msgs::msg::JointState *msg,
                                                           StateEstimator *est)
{
    nsec_ = msg->header.stamp.sec * 1e6 + msg->header.stamp.nanosec; // save nsecs for later.
    ...
    if(!jointStateFromROS(*msg, utime_, q_, qd_, qdd_, tau_)){
      RCLCPP_WARN_STREAM(LOGGER, "[LegodoHandlerROS::processMessage] Could not process joint state from ROS!");
      return nullptr;
    }
...
LegodoHandlerWithAccelerationROS::Update* LegodoHandlerWithAccelerationROS::processMessage(const pronto_msgs::msg::JointStateWithAcceleration *msg, StateEstimator *est)
{
    nsec_ = msg->header.stamp.sec * 1e6 + msg->header.stamp.nanosec; // save nsecs for later.
    ...
    if(!jointStateWithAccelerationFromROS(*msg, utime_, q_, qd_, qdd_, tau_)){
      RCLCPP_WARN_STREAM(LOGGER, "[LegodoHandlerWithAccelerationROS::processMessage] Could not process joint state from ROS!");
      return nullptr;
    }
    getPreviousState(est);
...
FootSensorLegodoHandlerROS::FootSensorLegodoHandlerROS(const rclcpp::Node::SharedPtr& node,
                                                       StanceEstimatorBase& stance_est,
                                                       LegOdometerBase& legodo)
  : LegodoHandlerBase(node, stance_est, legodo)
{
...
LegodoHandlerBase::Update * FootSensorLegodoHandlerROS::processMessage(const sensor_msgs::msg::JointState *msg, StateEstimator *est){
  nsec_ = msg->header.stamp.sec * 1e6 + msg->header.stamp.nanosec; // save nsecs for later.
  utime_ = nsec_ / 1000;  // A lot of internals still assume microseconds
  // TODO: transition from microseconds to nanoseconds everywhere
  if(!jointStateFromROS(*msg, utime_, q_, qd_, qdd_, tau_)){
    RCLCPP_WARN_STREAM(LOGGER, "[FootSensorLegodoHandlerROS::processMessage] Could not process joint state from ROS!");
    return nullptr;
  }
  getPreviousState(est);
...
ForceSensorLegodoHandlerROS::ForceSensorLegodoHandlerROS(const rclcpp::Node::SharedPtr& node,
                                                         StanceEstimatorBase& stance_est,
                                                         LegOdometerBase& legodo)
  : LegodoHandlerBase(node, stance_est, legodo)
{
...
LegodoHandlerBase::Update * ForceSensorLegodoHandlerROS::processMessage(const sensor_msgs::msg::JointState *msg, StateEstimator *est){
  nsec_ = msg->header.stamp.sec * 1e6 + msg->header.stamp.nanosec; // save nsecs for later.
  utime_ = nsec_ / 1000;  // A lot of internals still assume microseconds
  // TODO: transition from microseconds to nanoseconds everywhere
  if(!jointStateFromROS(*msg, utime_, q_, qd_, qdd_, tau_)){
    RCLCPP_WARN_STREAM(LOGGER, "[ForceSensorLegodoHandlerROS::processMessage] Could not extract joint states from ROS message!");
    return nullptr;
  }
  getPreviousState(est);
  // the data to compute the stance are processed in processSecondaryMessage()
...
bool ForceSensorLegodoHandlerROS::processMessageInit(const sensor_msgs::msg::JointState* /*msg*/, const std::map<std::string, bool>& /*sensor_initialized*/, const RBIS& /*default_state*/, const RBIM& /*default_cov*/, RBIS& /*init_state*/, RBIM& /*init_cov*/){
  return true;
}
